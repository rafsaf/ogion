...
!!! note
    _Any environment variable that starts with **"SINGLEFILE\_"** will be handled as Single File._ There can be multiple files paths definition for one ogion instance, for example `SINGLEFILE_FOO` and `SINGLEFILE_BAR`. Params must be included in value, splited by single space for example `"value1=1 value2=foo"`.
