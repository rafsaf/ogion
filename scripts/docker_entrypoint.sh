...
export OGION_CPU_ARCHITECTURE=${target_arch}
echo "`date` detected cpu arch: '$target_arch'"

if [ "$ROOT_MODE" = "false" ] 
then
  echo "`date` will start ogion as user: '$SERVICE_NAME' without root privilages"
  set -e
  ...
  echo "`date` finish docker_entrypoint.sh: run ogion as user '$SERVICE_NAME'"
  runuser -u ${SERVICE_NAME} -- $@
else
  echo "`date` finish docker_entrypoint.sh: run ogion as user: 'root'"
  $@
fi