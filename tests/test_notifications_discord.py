...
def test_limit_message_full_text_if_below_limit() -> None:
    limited_message = discord.Discord().limit_message(
        message="short message", limit=200
    )
...
def test_limit_message_limited_text_if_behind_limit() -> None:
    ...
    limited_message = discord.Discord().limit_message(message=long_text, limit=678)
    assert limited_message == "a" * (678 - len(trunc_text)) + trunc_text


def test_discord_skip_when_no_webhook_url() -> None:
    assert discord.Discord().send(message="text") is False
...
def test_discord_cases_when_webhook_url_in_config(
    ...
    with responses.RequestsMock() as rsps:
        ...
        monkeypatch.setattr(config.options, "DISCORD_WEBHOOK_URL", webhook_url)
        assert discord.Discord().send(message="text") == sent
